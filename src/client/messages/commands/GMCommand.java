...
public class GMCommand {
    ...
    public static class Ban extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 3) {
                c.getPlayer().dropMessage(5, "[Syntax] !" + getCommand() + " <�摰�> <����>");
                return 0;
            }
            ...
            if (target != null) {
                if (c.getPlayer().getGMLevel() > target.getGMLevel() || c.getPlayer().isAdmin()) {
                    ...
                    if (target.ban(sb.toString(), c.getPlayer().isAdmin(), false, hellban)) {
                        ...
                        c.getPlayer().dropMessage(6, "[" + getCommand() + "] 撠�仃���.");
                        return 0;
                    }
                } else {
                ...
                if (MapleCharacter.ban(splitted[1], sb.toString(), false, c.getPlayer().isAdmin() ? 250 : c.getPlayer().getGMLevel(), splitted[0].equals("!hellban"))) {
                    c.getPlayer().dropMessage(6, "[" + getCommand() + "] ���蝺��� " + splitted[1] + ".");
                    return 1;
                } else {
                    c.getPlayer().dropMessage(6, "[" + getCommand() + "] 撠�仃��� " + splitted[1]);
                    return 0;
                }
            }
        }
    ...
    public static class Giveagash extends 給點數 {
    }
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 3) {
                c.getPlayer().dropMessage(6, "!giveagash/!給點數[角色名稱] [數量].");
                return 0;
            }
            MapleCharacter chrs = c.getChannelServer
 
().getPlayerStorage().getCharacterByName(splitted[1]);
            if (chrs == null) {
                c.getPlayer().dropMessage(6, "請確認有在正確的頻道");
            } else {
                chrs.modifyCSPoints(1, Integer.parseInt(splitted[2]));
                c.getPlayer().dropMessage(6, "給予" + splitted[1] +" "
 
+ splitted[2]  + "點Gash點數 " + splitted[1] +"共擁有 " + 
 
chrs.getCSPoints(1) + " 點");
            }
            return 1;
        }
    }
    ...
    public static class Level extends CommandExecute {
       ...
       public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 2) {
                c.getPlayer().dropMessage(6, "雿輻閬��: !spy <�摰嗅���>");
            } else {
                MapleCharacter victim = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
                 if (victim.getGMLevel() > 3) {
                        c.getPlayer().dropMessage(5, "雿����������犖!");
                        return 0;
                    }
                if (victim != null) {
                    c.getPlayer().dropMessage(5, "甇斤摰嗥����:");
                    c.getPlayer().dropMessage(5, "蝑��: " + victim.getLevel() + "�璆�: " + victim.getJob() + "��: " + victim.getFame());
                    c.getPlayer().dropMessage(5, "����: " + victim.getMapId() +  " - " + victim.getMap().getMapName().toString());
                    c.getPlayer().dropMessage(5, "����: " + victim.getStat().getStr() + "  ||  ��: " + victim.getStat().getDex() + "  ||  ����: " + victim.getStat().getInt() + "  ||  撟賊��: " + victim.getStat().getLuk());
                    c.getPlayer().dropMessage(5, "���� " + victim.getMeso() + " 璆馳.");
                    victim.dropMessage(5, c.getPlayer().getName() + " GM�閫�撖..");
                } else {
                    c.getPlayer().dropMessage(5, "�銝甇斤摰�.");
                }
        }
            return 1;
    }
    ...
    public static class online1 extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.getPlayer().dropMessage(6, "銝��� ����-" + c.getChannel() + ":");
    ...
    public static class online extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
          ...
          c.getPlayer().dropMessage(6, "-------------------------------------------------------------------------------------");  
           c.getPlayer().dropMessage(6, new StringBuilder().append("����: ").append(c.getChannelServer().getChannel()).append(" 蝺�犖�: ").append(curConnected).toString());
            total += curConnected;
            ...
            c.getPlayer().dropMessage(6, new StringBuilder().append("���撩��蝮質���犖�: ").append(total).toString());
            c.getPlayer().dropMessage(6, "-------------------------------------------------------------------------------------");
            return 1;
        }
    }
    ...
    public static class CnGM extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
                ...
                World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(5, "<GM��予閬��>" + "����" + c.getPlayer().getClient().getChannel() + " [" + c.getPlayer().getName()+ "] : " + StringUtil.joinStringFrom(splitted, 1)).getBytes());
    ...
    public static class ClearInv extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (splitted[1].equals("��")) {
                ...
                c.getPlayer().dropMessage(6, "[��/撌脰���/甇血/瘨��/鋆ˇ/�隞�/�畾");
            }
